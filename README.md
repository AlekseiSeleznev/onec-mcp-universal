...
Визуальная панель мониторинга и управления:

```
http://localhost:8080/dashboard
```

**Вкладка "Информация":** состояние бэкендов, подключённые базы (со статусом EPF), профилирование запросов, кеш метаданных, статус анонимизации, Docker-контейнеры.

**Вкладка "Параметры":** управление базами (добавить / изменить / отключить / установить по умолчанию), редактирование `.env` с автоперезапуском шлюза, очистка кеша, переключение анонимизации.

**Документация:** `http://localhost:8080/dashboard/docs` -- подробное описание всех инструментов, API-эндпоинтов и переменных окружения.

**Диагностика:** ссылка внизу дашборда -- полный отчёт о состоянии системы (версии, бэкенды, базы, Docker, логи контейнеров).
