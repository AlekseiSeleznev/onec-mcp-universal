...
- **Веб-дашборд** -- панель на `http://localhost:8080/dashboard`: статус, настройки, отчёты и встроенная документация RU/EN.
...
- **Отчёты по пользовательскому названию** -- `find_reports`, `describe_report` и `run_report` работают с названием из интерфейса 1С, а не только с техническим именем. `enrich_report_docs` добавляет описания из ИТС/1С:Напарника. Каталог хранится в SQLite отдельно по каждой базе; дашборд показывает только сводку и сохранённые запуски.
...
В интерфейсе есть `Статус`, `Отчёты` и `Настройки`. Поиск и запуск отчётов в основном рассчитаны на MCP/API, а на вкладке `Статус` остаётся короткая техническая сводка.
...
- анонимизация;
- `Отчёты 1С`: когда строился каталог, сколько найдено отчётов и вариантов, сколько запусков уже сохранено и как распределились их статусы;
