...
MCP-шлюз для работы с 1С:Предприятие из AI-ассистентов и любых MCP-клиентов.

Вместо нескольких отдельных MCP-подключений используется один адрес: `http://localhost:8080/mcp`. Шлюз принимает запросы от AI-клиента и отправляет их в нужный бэкенд: данные 1С, документацию платформы, BSL-навигацию, отчёты или служебные операции. У каждого сеанса AI-ассистента своя активная база, поэтому два окна Codex могут спокойно работать с разными базами.
...
- [Запуск отчётов 1С по пользовательскому названию](#запуск-отчётов-1с-по-пользовательскому-названию)
  ...
  - [Два способа запуска: API и веб-интерфейс](#два-способа-запуска-api-и-веб-интерфейс)
  - [Контракты результата](#контракты-результата)
  - [Как получать данные из отчётов](#как-получать-данные-из-отчётов)
  - [Политики запуска и сценарии веб-интерфейса](#политики-запуска-и-сценарии-веб-интерфейса)
  - [Статусы выполнения отчётов](#статусы-выполнения-отчётов)
  - [Настройки отчётов на дашборде](#настройки-отчётов-на-дашборде)
...
| **Отчёты 1С** | 15 | Поиск, описание, запуск, политики API/веб-интерфейса, сценарии открытия, сохранённые результаты и контрактная проверка |
...
- **Маршрутизация по сессиям** -- каждый MCP-сеанс держит свою активную БД.
- **Автоподключение из EPF** -- нажмите "Подключиться" в `MCPToolkit.epf`, и база зарегистрируется в шлюзе сама.
- **Удалённый режим** -- шлюз может работать на сервере 1С, а AI-ассистент на рабочей станции разработчика.
- **Анонимизация ПД** -- ФИО, ИНН, СНИЛС, телефоны и email маскируются со стабильным hash-маппингом.
- **1С:Напарник** -- поиск по ИТС через API `code.1c.ai`.
- **Поиск по BSL** -- индекс процедур и функций конфигурации.
- **Профилирование запросов** -- замер времени выполнения и подсказки по проблемным запросам.
- **Кеш метаданных** -- TTL-кеш для `get_metadata`, по умолчанию 10 минут.
- **Веб-дашборд** -- панель на `http://localhost:8080/dashboard`: статус, настройки и встроенная документация RU/EN.
- **Просмотрщик графа** -- `http://localhost:8888/` в профиле `bsl-graph`: канвас на `cytoscape.js`, раскрытие двойным кликом, поиск по нескольким словам (`размеры пособий` → `РазмерыГосударственныхПособий`), история поиска, список результатов и выбор активной БД.
- **Отчёты по пользовательскому названию** -- `find_reports`, `describe_report` и `run_report` работают с названием из интерфейса 1С, а не только с техническим именем. Есть два независимых контура запуска: быстрый API-контур через `toolkit`/`EPF` и дополнительный контур через веб-клиент 1С с экспортом результата в файл. Каталог, политики запуска, сценарии веб-интерфейса, контракты результата и история запусков хранятся в SQLite отдельно по каждой базе; дашборд показывает локальную сводку и настройки, но сам отчёты не запускает.
- **Редактирование `.env`** -- из дашборда, с автоматическим перезапуском шлюза.
- **96 скилов для Codex и совместимых локальных раннеров навыков** -- объекты, формы, обработки, отчёты, расширения и процессные сценарии.
...
- **README / CODEX.md** -- пользовательская документация и отдельная памятка для Codex.
...
План пользовательского и процессного слоя вынесен в отдельный документ:
...
**Маршрутизация по сессиям:** каждый AI-клиент получает уникальный `Mcp-Session-Id`. Шлюз маршрутизирует запросы к активной базе данных этой сессии. Разные сеансы Codex и других MCP-клиентов могут одновременно работать с разными базами. Таймаут простоя сессии -- 8 часов.
...
**Безопасность:** при каждом свежем открытии `MCPToolkit.epf` все флаги опасных операций по умолчанию выключены. В форме доступны три независимые опции:
- `Записать объект`
- `Привилегированный режим`
- `Все опасные операции без предупреждения`

Последняя отключает общий шлюз подтверждения опасных операций и при переключении показывает отдельное предупреждение. На рабочих базах рекомендуется оставить все три чекбокса выключенными.
...
Индикатор EPF основан на heartbeat (`/api/epf-heartbeat`): если сигнал не приходит дольше `EPF_HEARTBEAT_TTL_SECONDS`, индикатор становится жёлтым.
...
Маршрутизация по сессиям: если один сеанс Codex работает с ERP, а другой -- с ZUP, они не пересекаются. Таймаут простоя сессии -- 8 часов.
...
Шлюз строит отдельный для каждой базы каталог отчётов в SQLite (`/data/report-catalog.sqlite`) по выгруженным BSL/XML-исходникам. Каталог хранит пользовательские названия, технические имена, варианты, параметры, стратегии запуска, контракты результата, политики выбора движка и историю запусков. Поэтому отчёт можно найти и запустить по названию, которое видит пользователь в интерфейсе 1С, без знания технического имени объекта.

### Архитектура контура отчётов

Контур отчётов разделён на несколько уровней:

- `report_catalog` ведёт локальный SQLite-каталог, варианты отчётов, параметры, контракты результата, политики движков, сценарии веб-интерфейса, кампании проверки и историю запусков;
- `report_analyzer` разбирает выгруженные BSL/XML, формы, менеджеры, настройки СКД и подсказки графа зависимостей, чтобы понять, какие отчёты существуют, какие варианты есть и какую структуру результата можно ожидать;
- `report_runner` выполняет быстрый запуск без браузера через контур `toolkit`/`EPF`;
- `report_ui_runner` выполняет отчёт через веб-клиент 1С, сохраняет результат во временный файл и передаёт его в разбор;
- `report_result_extractor` читает экспортированный файл, нормализует строки, колонки, итоги и строит наблюдаемую структуру результата;
- `report_orchestrator` выбирает контур запуска (`api`, `ui` или `auto`), учитывая глобальные настройки, политику конкретного отчёта и результат контрактной проверки;
- `tool_handlers/reports` публикует MCP/API-инструменты поверх этого контура.

Каталог и результаты запусков живут внутри контейнера шлюза: каталог в `/data/report-catalog.sqlite`, сохранённые результаты в `/data/report-results`. Это локальное состояние шлюза. Выполнение отчёта идёт либо через подключённую обработку `MCPToolkit.epf`, либо через веб-клиент 1С, если включён контур через веб-интерфейс.
...
- `describe_report(database, title|report, variant)` -- показать найденный отчёт, варианты, параметры, стратегию запуска и `output_contract`;
- `get_report_runner_policy(database, report, variant)` -- получить сохранённую политику выбора `api`/`ui`/`auto`;
- `set_report_runner_policy(database, report, variant, preferred_runner, api_enabled, ui_enabled)` -- сохранить предпочтительный движок для отчёта или варианта;
- `get_report_ui_strategy(database, report, variant)` -- получить сохранённый сценарий открытия, заполнения параметров и экспорта в веб-клиенте;
- `set_report_ui_strategy(database, report, variant, strategy)` -- сохранить точный сценарий веб-запуска для отчёта или варианта;
- `run_report(database, title|report, period, filters, params, context, runner)` -- выполнить отчёт и вернуть данные либо формализованный запрос недостающего контекста;
- `validate_all_reports(database)` -- быстрый последовательный прогон каталога;
- `validate_report_contracts(database)` -- тяжёлая проверка вариантов отчётов с сравнением ожидаемой и фактической структуры результата;
- `get_report_result(database, run_id)` -- получить страницу большого сохранённого результата;
- `get_report_validation_campaign(database, campaign_id)` -- прочитать сохранённую кампанию контрактной проверки;
- `explain_report_strategy(database, title, variant)` -- объяснить, почему модуль запуска выбрал конкретную стратегию, и показать последнюю контрактную диагностику.

Для HTTP-клиентов поверх этого же контура доступна REST-обёртка `POST /api/reports/{action}` с действиями `analyze`, `find`, `list`, `describe`, `runner-policy`, `set-runner-policy`, `ui-strategy`, `set-ui-strategy`, `run`, `validate-all`, `validate-contracts`, `result`, `campaign`, `explain`. Она повторяет контракт соответствующих MCP-инструментов для отчётов и не зависит от видимости карточек в дашборде.
...
### Два способа запуска: API и веб-интерфейс

У отчётного контура есть два независимых движка.

**API-движок** работает без браузера. Он использует контур `toolkit`/`EPF`, быстрее стартует, подходит для массовых проверок и остаётся основным способом запуска. Если отчёт можно корректно сформировать и получить как строки через этот контур, UI не нужен.

**UI-движок** работает через веб-клиент 1С. Он открывает отчёт так же, как пользователь, заполняет период, отборы и параметры, нажимает `Сформировать`, сохраняет результат во временный файл и разбирает его. Основной формат экспорта -- `XLSX`, запасной -- `HTML`; чтение DOM используется как резервный путь. Скриншоты сохраняются только для диагностики ошибок, а не как основной источник данных.

Временный файл после успешного разбора удаляется. Он остаётся на диске только если включено хранение файлов ошибок UI или если разбор упал и файл нужен для диагностики. UI-движок работает только на чтение: он не записывает, не проводит и не удаляет объекты 1С.

Параметр `runner` в `run_report` принимает значения:

- `api` -- использовать только API-движок;
- `ui` -- использовать только веб-интерфейс;
- `auto` -- сначала применить политику отчёта и глобальные настройки, затем при разрешённом переходе на UI попробовать веб-интерфейс, если API технически не смог получить корректную табличную часть.

Контур через веб-интерфейс опционален. Проект продолжает работать без веб-публикации базы и без Playwright/web-test: в этом случае `runner=api` и все остальные MCP-инструменты остаются доступными.

### Контракты результата

Для отчёта или варианта каталог хранит ожидаемую структуру результата:

- `declared` -- контракт, который анализатор вывел из BSL/XML, СКД, форм, модулей объекта и менеджера;
- `verified` -- контракт, зафиксированный после подтверждённого успешного запуска.

Контракт описывает не конкретные значения, а форму результата: тип вывода (`rows`, `artifact`, `mixed`, `unknown`), наличие детальных строк, колонок, группировок, итогов, иерархии и смысловые роли полей. После каждого запуска строится `observed_signature`: какие колонки реально пришли, есть ли табличная часть, итоги, служебные заголовки и артефакты.

Сравнение `expected vs observed` помогает отличать разные классы проблем:

- отчёт вернул только шапку, без детальных строк;
- выбран не тот вариант или не та стратегия запуска;
- API вернул артефакт вместо строк;
- статический контракт слабый и требует уточнения;
- отчёт требует контекста, который нельзя подобрать автоматически.

`describe_report` возвращает поле `output_contract`, а `explain_report_strategy` показывает последнюю информацию о совпадении стратегии с контрактом. Тяжёлая проверка `validate_report_contracts` идёт по вариантам отчётов, сохраняет кампанию и помечает каждую цель проверки как совпавшую или отложенную с понятной причиной.
...
2. Для найденного кандидата вызвать `describe_report(database, title)` и посмотреть техническое имя, варианты, параметры, ожидаемую структуру результата и доступную стратегию запуска.
3. Выбрать движок. Обычно достаточно `runner=auto`. Если нужен конкретный путь, указать `runner=api` или `runner=ui`.
4. Запустить `run_report(...)` с явным `database` и, при необходимости, передать:
...
5. Если ответ большой, дочитывать его через `get_report_result(database, run_id)`.
6. Если нужно понять выбор стратегии, переход на UI или причину ограничения, вызвать `explain_report_strategy(...)`.

На уровне дашборда отчёты не запускаются. В интерфейсе на вкладке `Статус` показывается только диагностическая сводка: построен ли каталог, когда был последний анализ, сколько отчётов и вариантов найдено, сколько запусков и сохранённых результатов есть в локальном состоянии шлюза, и как распределяются последние статусы.

### Политики запуска и сценарии веб-интерфейса

Для конкретного отчёта или варианта можно сохранить политику запуска:

```json
{
  "database": "MY_INFOBASE",
  "report": "TechnicalReportName",
  "variant": "Main",
  "preferred_runner": "ui",
  "api_enabled": true,
  "ui_enabled": true,
  "reason": "API returns an artifact without detail rows"
}
```

Политика применяется в режиме `runner=auto`. Если пользователь явно передал `runner=api` или `runner=ui`, явный выбор имеет приоритет над сохранённой политикой.

UI-сценарий хранит точный способ открыть отчёт, сопоставить параметры с полями формы, нажать кнопку формирования и выполнить экспорт:
...
```json
{
  ...
  "variant": "Main",
  "strategy": {
    "open": {"mode": "metadata_link", "metadata_path": "Отчет.TechnicalReportName"},
    "parameter_map": {
...
Если сохранённого сценария нет, UI-движок использует общий сценарий по ссылке метаданных `Отчет.<ИмяОтчета>`. После успешного подтверждённого запуска шлюз может сохранить проверенный сценарий и проверенный контракт, чтобы следующий запуск не проходил лишнюю разведку формы.
...
- `error` -- необработанная ошибка выполнения. Это дефект, регрессия или сбой окружения, а не ожидаемый исход.
...
### Настройки отчётов на дашборде

На вкладке `Настройки` есть карточка `Движок отчётов 1С`. Она меняет значения по умолчанию для MCP/API, но не запускает отчёты сама:

- `Автоанализ после подключения, выгрузки и переиндексации BSL` -- перестраивать каталог после событий, которые меняют BSL-исходники;
- `Использовать API-движок` -- разрешить быстрый контур через `toolkit`/`EPF`;
- `Использовать UI-движок` -- разрешить запуск через веб-клиент 1С;
- `Переходить на UI при техническом ограничении API` -- в режиме `runner=auto` пробовать UI, если API не смог вернуть корректную табличную часть;
- `Формат экспорта UI` -- формат временного файла для UI-движка, сейчас `XLSX` или `HTML`;
- `Хранить файлы ошибок UI` -- оставлять временные файлы только для диагностики неудачных UI-запусков;
- `Строк при запуске` и `Таймаут запуска` -- значения по умолчанию для `run_report`;
- `Строк при проверке` и `Таймаут проверки` -- значения по умолчанию для массовой проверки.

На вкладке `Статус` карточка `Отчёты 1С` читает только `/data/report-catalog.sqlite` и `/data/report-results`. Она не вызывает `analyze_reports`, не запускает отчёты и не обращается к среде выполнения 1С при открытии страницы.
...
Дашборд доступен здесь:

```
http://localhost:8080/dashboard
```

В интерфейсе есть вкладки `Статус` и `Настройки`. Поиск, описание и запуск отчётов выполняются через MCP/API, а дашборд показывает только локальную сводку и настройки отчётного контура.

### Вкладка "Статус"

На этой вкладке видно текущее состояние шлюза:
- бэкенды и количество доступных инструментов;
- подключённые базы и статус EPF;
- состояние backend-подключения базы: именно оно управляет зачёркиванием имени и кнопками `Отключить` / `Подключить`;
- профилирование запросов: количество вызовов и время выполнения;
- кеш метаданных: размер, TTL, попадания и промахи;
- анонимизация;
- `Отчёты 1С`: когда строился каталог, сколько найдено отчётов и вариантов, сколько запусков и результатов сохранено, как распределились последние статусы;
- Docker-контейнеры и базовые ресурсы.

Кнопка `Обновить статистику` отдельно перечитывает RAM контейнеров и размеры образов на диске. При обычной загрузке страница не делает тяжёлый `docker stats`, чтобы дашборд открывался быстро.
...
Здесь собраны действия, которые обычно нужны при настройке:
- добавить, изменить, отключить базу или выбрать базу по умолчанию;
- отредактировать `.env` из браузера, после сохранения шлюз перезапустится сам;
- очистить TTL-кеш метаданных;
- включить или выключить маскировку ПД;
- настроить движок отчётов 1С: автоанализ каталога, API-движок, UI-движок, переход на UI при техническом ограничении API, формат UI-экспорта, хранение файлов ошибок, лимиты строк и таймауты.
...
Встроенная документация описывает MCP-инструменты, API-эндпоинты, переменные окружения и типовые проблемы. Есть русская и английская версии.

### Диагностика

Ссылка "Диагностика" внизу дашборда открывает JSON-отчёт: версия шлюза, активные сессии, бэкенды, базы, профилирование, кеш, Docker, последние строки логов контейнеров и переменные окружения.
