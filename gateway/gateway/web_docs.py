...
<span style="color:#22c55e">&#9679;</span> зеленый - обработка запущена в клиенте 1С, long-polling активно, AI может выполнять запросы.<br>
...
<tr><td><b>Подключиться</b></td><td>Выполняет два действия:<br>1. <b>Регистрация</b> - отправляет данные базы (имя, строку подключения, текущий <code>channel</code> экземпляра формы) на шлюз через <code>/api/register</code>. Если база ещё не подключена, шлюз автоматически создаёт Docker-контейнеры.<br>2. <b>Long-polling</b> - устанавливает постоянное соединение с toolkit-бэкендом. Обработка начинает «слушать» входящие команды от AI и выполнять их в контексте текущей информационной базы.</td></tr>
...
<tr><td><b>Все опасные операции без предупреждения</b></td><td>Глобально отключает предупреждения для dangerous operations. При включении форма показывает отдельный вопрос-подтверждение с перечислением действий: запись, удаление, файловые операции, COM/add-in вызовы, привилегированный и монопольный режим.</td><td>На production-базах - <b>выключен</b>. Включать только для контролируемых тестовых сценариев.</td></tr>
</table>
<div class="warn"><p><b>Безопасность:</b> На production-базах с реальными данными рекомендуется оставить все три чекбокса выключенными. Каждая потенциально опасная операция будет требовать ручного подтверждения оператором в клиенте 1С. Это предотвращает случайное удаление или модификацию данных.</p></div>
...
<tr><td><code>execute_code</code></td><td>Выполнение произвольного кода на встроенном языке 1С. Поддерживает <code>execution_context=\"server\"</code> для CRUD/метаданных и <code>execution_context=\"client\"</code> для реквизитов формы EPF и dangerous-toggle checks. Возврат идёт через переменную <code>Результат</code>.</td></tr>
...
<tr><td><code>get_link_of_object</code></td><td>Генерация навигационной ссылки на объект. В live API ожидает аргумент <code>object_description</code> с object-ref descriptor.</td></tr>
<tr><td><code>find_references_to_object</code></td><td>Поиск всех мест использования объекта. В live API ожидает <code>target_object_description</code> и <code>search_scope</code>.</td></tr>
...
<tr><td><code>graph_related</code></td><td>Поиск связанных объектов во встроенном графе зависимостей: что использует объект и что его использует (impact analysis). Требует профиль <code>bsl-graph</code>.</td></tr>
</table>
