...
def _pick(
    ...
    *,
    ...
    forbidden_initial: str | None = None,
) -> str:
    start = _stable_hash(value, salt) % len(pool)
    for offset in range(len(pool)):
        candidate = pool[(start + offset) % len(pool)]
        if forbidden and candidate == forbidden:
            continue
        if forbidden_initial and candidate.startswith(forbidden_initial):
            continue
        return candidate
    return pool[start]
...
class Anonymizer:
    """Stateful anonymizer with stable mapping for the configured salt."""

    def __init__(self, salt: str | None = None) -> None:
        self.enabled: bool = False
        normalized_salt = (salt or "").strip()
        self._salt = normalized_salt or secrets.token_hex(32)
    ...
    def _replace_fio(self, match: re.Match) -> str:
        ...
        fake = (
            f"{_pick(_LAST_NAMES, original, f'{self._salt}:l', forbidden=last_name)} "
            f"{_pick(_FIRST_NAMES, original, f'{self._salt}:f', forbidden=first_name)} "
            f"{_pick(_MIDDLE_NAMES, original, f'{self._salt}:m', forbidden=middle_name)}"
        )
        self._cache[original] = fake
        return fake
    ...
    def _replace_fio_short(self, match: re.Match) -> str:
        ...
        ln = _pick(_LAST_NAMES, original, f"{self._salt}:l", forbidden=last_name)
        fn = _pick(_FIRST_NAMES, original, f"{self._salt}:f", forbidden_initial=first_initial)
        mn = _pick(_MIDDLE_NAMES, original, f"{self._salt}:m", forbidden_initial=middle_initial)
        fake = f"{ln} {fn[0]}.{mn[0]}."
        self._cache[original] = fake
        return fake
    ...
    def _replace_company(self, text: str) -> str:
        ...
        def _repl(m: re.Match) -> str:
            ...
            if original in self._cache:
                return self._cache[original]
            prefix = m.group(1)
            fake = f'{prefix} "{_pick(_COMPANY_PREFIXES, original, f"{self._salt}:c")}"'
            self._cache[original] = fake
            return fake
        return re.sub(
...
# Singleton
anonymizer = Anonymizer(salt=settings.anonymizer_salt or None)
