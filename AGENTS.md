...
`onec-mcp-universal` — нейтральный к AI-клиенту MCP-шлюз для 1С:Предприятия.
...
## Инварианты рабочего каталога и экспорта

- путь BSL-рабочего каталога, заданный из дашборда, — источник истины
- `BSL_HOST_WORKSPACE` / `BSL_WORKSPACE` в `.env` определяют корневой каталог экспорта на хосте
- сервис хостовой выгрузки обязан читать актуальный `.env` или явный `output_dir` запроса
- автоиндексация после экспорта должна предпочитать путь хоста, видимый из gateway, и только потом использовать запасной путь из LSP/контейнера

## Инварианты жизненного цикла базы

- `disconnect` останавливает рабочий контур базы, но оставляет запись в реестре
- `remove` очищает рабочий контур, состояние и граф базы, но не удаляет физические BSL-файлы
- удаление базы не должно возвращать успех до завершения очистки рабочего контура

## Политика публикации

- `ghcr.io/alekseiseleznev/onec-mcp-universal:latest` отслеживает `main`
- release tags публикуют semver-теги образов

## Совместимость

- проект нейтрален к конкретному AI-клиенту
- любой MCP-клиент может подключаться вручную к тому же адресу
...
- **Распознавание намерения.** Фразы «используем 1С `<имя>`», «работаем с 1С `<имя>`», «подключись к 1С `<имя>`», «1С / 1C / онек», любые 1С-термины (BSL, Справочник, Документ, Регистр, Конфигурация, БСП, ИТС, `Srvr=…;Ref=…;`, `File=…`, имена баз `Z01`/`ZUP`/`ERP`/`БП`) → **этот MCP**. При упоминании базы: `list_databases` → если есть, `switch_database`; иначе попросить строку подключения и `connect_database`. Если пользователь сказал «база X» без указания системы — `list_databases`; если есть — работаем, если нет — честно сказать «в 1С-MCP такой базы нет», не выдумывать.
...
- **Скиллы** для Codex и совместимых локальных раннеров навыков ставятся `install-skills.sh` / `install-skills.ps1`. Скрипты в `skills/*/scripts/` — локальный автоматизированный путь, ориентированный на Codex.
