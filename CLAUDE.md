...
BSL / язык 1С, Справочник / Документ / Регистр / Перечисление / Отчёт / Обработка / БизнесПроцесс / ПланВидов\*, Конфигурация, БСП, ИТС, 1С:Напарник, MCPToolkit.epf, connection strings вида `Srvr=...;Ref=...;` или `File=...`.
...
2. Если нет — спросить у пользователя connection string и вызвать `connect_database`. Не угадывать.
...
2. `write_bsl` — запись в workspace (автоматически триггерит reindex + обновление графа).
...
- Если запрос касается 1С-платформенного API, которого нет ни в ИТС ни в подключённой конфигурации — **честно скажи**: «я не нашёл упоминаний X ни в workspace, ни в ИТС — уточните версию платформы / БСП / где видели это». Не фантазируй.
...
- Runbook для Codex: [`CODEX.md`](CODEX.md).
