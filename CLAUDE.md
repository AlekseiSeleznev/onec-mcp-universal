...
### 4. Перед получением данных отчёта

1. В первую очередь используй инструменты отчётов `onec-mcp-universal`, а не прямой запрос к регистрам.
2. `find_reports(database, query)` или `list_reports(database)` — найти отчёт по пользовательскому названию или просмотреть каталог.
3. `describe_report(database, title|report|variant)` — уточнить вариант, параметры и стратегию запуска.
4. `run_report(database, title|report, period, filters, params, context)` — получить данные отчёта.
5. `get_report_result(database, run_id)` — дочитать большие результаты постранично.
6. Если `run_report` вернул `needs_input`, дозаполни только явно запрошенные поля и повтори `run_report`.
7. Для всех report-tools `database` обязателен; не полагайся на активную БД сессии.

### 5. Перед редактированием BSL-модуля
...
### 7. Граф зависимостей / анализ влияния
...
### 8. Работа с конфигурацией без прямого подключения
...
- Если ты в Claude Code — используй MCP-инструменты (`write_bsl`, `export_bsl_sources`, `reindex_bsl`). Прямой запуск shell-скриптов из `skills/` нужен только для slash-команд Codex.

### 9. Когда MCP не помогает
...
3. **Несколько БД и параллельная работа** — активная база привязана к `Mcp-Session-Id`. `switch_database` делается один раз на сессию. Две параллельные сессии (Claude Code + Codex, например) могут держать **разные** активные БД одновременно — они полностью изолированы; у каждой БД свой toolkit-контейнер (порты 6100, 6101…) и свой LSP-контейнер.
