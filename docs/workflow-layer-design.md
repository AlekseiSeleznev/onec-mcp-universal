# Дизайн процессного слоя в `onec-mcp-universal`
...
Workflow-слой добавляет именно этот пользовательский и процессный слой, не создавая отдельный параллельный стек.
...
- процессные скиллы оркестрируют действия, а не дублируют низкоуровневые возможности;
...
## 3. Что входит в процессный слой
...
- процессный уровень фичи;
- вспомогательные процессные входы для типовых задач 1С.
...
Если техническая возможность уже существует в MCP или в низкоуровневом skill, процессный скилл должен:
...
MCP-инструменты остаются каноническим API рабочего контура.
...
- документация процессного слоя не должна зависеть от бренда конкретного клиента.

## 5. Основные блоки процессного слоя
...
- создаёт минимальную заготовку для процессного слоя;
...
Не все эти skills одинаково «высокоуровневые», но вместе они образуют повторяемый процессный слой над MCP рабочего контура.
...
Поэтому процессный слой должен оставаться встроенным в `onec-mcp-universal` и развиваться вместе с ним.

## 10. Что проверять при изменениях процессного слоя

Если меняется любой process-skill или документ этого слоя, нужно проверить:
...
7. что процессные скиллы по-прежнему оркестрируют существующие возможности, а не дублируют их.
