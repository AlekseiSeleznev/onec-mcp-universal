...
Процедура Подключиться(Команда)
	...
	Если ПустаяСтрока(АдресСервера) Тогда
		УстановитьСтатус("Ошибка: адрес сервера не определён", Ложь);
		ДобавитьВЛог("Ошибка: адрес сервера не определён после регистрации в шлюзе");
		Возврат;
	КонецЕсли;
...
Процедура Отключиться(Команда)
	ОтключитьсяОтСервера();
	// Уведомить шлюз об отключении EPF
	Попытка
		АдресGateway = СокрЛП(ГейтвейURL);
...
Процедура ОткрытьДашборд(Команда)
	АдресGateway = СокрЛП(ГейтвейURL);
	Если ПустаяСтрока(АдресGateway) Тогда
		АдресGateway = "http://localhost:8080";
	КонецЕсли;
	ЗапуститьПриложение(АдресGateway + "/dashboard");
КонецПроцедуры
...
Процедура ВыгрузитьBSL(Команда)
	...
	Попытка
		...
		Если СтруктураURL.Протокол = "https" Тогда
			Соединение = Новый HTTPСоединение(СтруктураURL.Хост, СтруктураURL.Порт, , , , 600, Новый ЗащищенноеСоединениеOpenSSL());
		Иначе
			Соединение = Новый HTTPСоединение(СтруктураURL.Хост, СтруктураURL.Порт, , , , 600);
		КонецЕсли;
		...
		Если Ответ.КодСостояния = 200 Тогда
			ДобавитьВЛог("Выгрузка завершена.");
			ДобавитьВЛог("Ответ: " + Ответ.ПолучитьТелоКакСтроку());
		Иначе
			ДобавитьВЛог("Ошибка HTTP " + Ответ.КодСостояния + ": " + Ответ.ПолучитьТелоКакСтроку());
		КонецЕсли;
	Исключение
...
Процедура ЗарегистрироватьсяВШлюзе(Команда)
	...
	ТелоJSON = "{""name"": """ + ИмяБазыДляРегистрации + """, ""connection"": """ + СтрокаПодключенияРег + """}";

	Попытка
		СтруктураURL = РазобратьURL(АдресGateway);
		Если СтруктураURL.Протокол = "https" Тогда
			Соединение = Новый HTTPСоединение(СтруктураURL.Хост, СтруктураURL.Порт, , , , 60, Новый ЗащищенноеСоединениеOpenSSL());
		Иначе
			Соединение = Новый HTTPСоединение(СтруктураURL.Хост, СтруктураURL.Порт, , , , 60);
		КонецЕсли;
		...
		Если Ответ.КодСостояния = 200 Тогда
			...
			Если НачалоURL > 0 Тогда
				...
				Если КонецURL > НачалоURL Тогда
					...
					НовыйАдресСервера = СтрЗаменить(НовыйАдрес, "/1c/poll", "");
