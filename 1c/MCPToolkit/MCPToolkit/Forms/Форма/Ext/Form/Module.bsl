...
Процедура ПриОткрытии(Отказ)
	...
	// Автоматическая регистрация в шлюзе при открытии
...
Процедура ОпределитьПараметрыБазы(Команда)
	...
	Если ПустаяСтрока(СерверБазы) Тогда
		ДобавитьВЛогВыгрузки("Сервер не определён — возможно, файловая база.");
	Иначе
		ДобавитьВЛогВыгрузки("Сервер: " + СерверБазы + ", база: " + ИмяБазы);
	КонецЕсли;
КонецПроцедуры
...
Процедура ВыгрузитьBSL(Команда)
	Если ПустаяСтрока(СокрЛП(СерверБазы)) ИЛИ ПустаяСтрока(СокрЛП(ИмяБазы)) Тогда
		ДобавитьВЛогВыгрузки("Ошибка: заполните параметры подключения к базе или нажмите «Определить автоматически».");
		Возврат;
	КонецЕсли;
	...
	ДобавитьВЛогВыгрузки("Запуск выгрузки BSL... Подключение: Srvr=" + СерверТримм + ";Ref=" + БазаТримм + ";");
	...
	Попытка
		...
		Если ПустаяСтрока(СтруктураURL.Хост) Тогда
			ДобавитьВЛогВыгрузки("Ошибка: неверный адрес MCP Gateway.");
			Возврат;
		КонецЕсли;
		...
		Если Ответ.КодСостояния = 200 Тогда
			ДобавитьВЛогВыгрузки("Выгрузка завершена.");
			ДобавитьВЛогВыгрузки("Ответ: " + Ответ.ПолучитьТелоКакСтроку());
		Иначе
			ДобавитьВЛогВыгрузки("Ошибка HTTP " + Ответ.КодСостояния + ": " + Ответ.ПолучитьТелоКакСтроку());
		КонецЕсли;
	Исключение
		ДобавитьВЛогВыгрузки("Ошибка соединения с Gateway: " + ОписаниеОшибки());
	КонецПопытки;
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьВЛогВыгрузки(Сообщение)
	ЛогВыгрузки = ЛогВыгрузки + Формат(ТекущаяДата(), "ДФ=HH:mm:ss") + "  " + Сообщение + Символы.ПС;
КонецПроцедуры
...
Процедура ЗарегистрироватьсяВШлюзе(Команда)
	...
	Если ПустаяСтрока(ИмяБазыДляРегистрации) Тогда
		ДобавитьВЛогВыгрузки("Ошибка: имя базы не заполнено. Нажмите «Определить автоматически».");
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		Ответ = Соединение.ОтправитьДляОбработки(Запрос);
		Если Ответ.КодСостояния = 200 Тогда
			ДобавитьВЛогВыгрузки("Зарегистрировано в шлюзе: база """ + ИмяБазыДляРегистрации + """");

			// Переключаемся на toolkit URL, выданный шлюзом (динамический порт)
			Попытка
				ДанныеОтвета = ПрочитатьJSON2(Ответ.ПолучитьТелоКакСтроку());
				НовыйАдрес = ДанныеОтвета.Получить("toolkit_poll_url");
				Если НовыйАдрес <> Неопределено И НЕ ПустаяСтрока(НовыйАдрес) Тогда
					// Убираем /1c/poll — нам нужен базовый адрес сервера
					НовыйАдресСервера = СтрЗаменить(НовыйАдрес, "/1c/poll", "");
					Если НовыйАдресСервера <> АдресСервера Тогда
						ДобавитьВЛогВыгрузки("Переключение toolkit: " + АдресСервера + " → " + НовыйАдресСервера);
						АдресСервера = НовыйАдресСервера;
						// Если уже подключены — переподключаемся к новому toolkit
						Если ФлагПодключения Тогда
							ОтключитьсяОтСервера();
						КонецЕсли;
					КонецЕсли;
					// Автоподключение после регистрации
					Если НЕ ФлагПодключения Тогда
						Подключиться(Неопределено);
					КонецЕсли;
				КонецЕсли;
			Исключение
				// Не критично — продолжаем без переключения
			КонецПопытки;
		Иначе
			ДобавитьВЛогВыгрузки("Ошибка регистрации HTTP " + Ответ.КодСостояния + ": " + Ответ.ПолучитьТелоКакСтроку());
		КонецЕсли;
	Исключение
		ДобавитьВЛогВыгрузки("Ошибка подключения к шлюзу: " + ОписаниеОшибки());
	КонецПопытки;
КонецПроцедуры
