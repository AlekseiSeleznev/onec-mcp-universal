...
Процедура ОпределитьПараметрыБазы(Команда)
	Параметры = ПолучитьПараметрыТекущейБазыНаСервере();
	СерверБазы = Параметры.Сервер;
...
Процедура ВыгрузитьBSL(Команда)
	Если ПустаяСтрока(АдресСервера) Тогда
		ДобавитьВЛогВыгрузки("Ошибка: не указан адрес MCP-сервера (вкладка Подключение).");
		Возврат;
	КонецЕсли;
	Если ПустаяСтрока(СерверБазы) ИЛИ ПустаяСтрока(ИмяБазы) Тогда
		ДобавитьВЛогВыгрузки("Ошибка: заполните параметры подключения к базе или нажмите «Определить автоматически».");
		Возврат;
	КонецЕсли;

	СтрокаПодключения = "Srvr=" + СерверБазы + ";Ref=" + ИмяБазы + ";";
	Если НЕ ПустаяСтрока(ПользовательBSL) Тогда
		СтрокаПодключения = СтрокаПодключения + "Usr=" + ПользовательBSL + ";";
	КонецЕсли;
	Если НЕ ПустаяСтрока(ПарольBSL) Тогда
		СтрокаПодключения = СтрокаПодключения + "Pwd=" + ПарольBSL + ";";
	КонецЕсли;

	ДобавитьВЛогВыгрузки("Запуск выгрузки BSL... Подключение: Srvr=" + СерверБазы + ";Ref=" + ИмяБазы + ";");
	...
	Попытка
		...
		Если ПустаяСтрока(СтруктураURL.Хост) Тогда
			ДобавитьВЛогВыгрузки("Ошибка: неверный адрес MCP-сервера.");
			Возврат;
		КонецЕсли;
		...
		Если Ответ.КодСостояния = 200 Тогда
			ДобавитьВЛогВыгрузки("Выгрузка запущена на сервере. Ожидайте завершения (может занять несколько минут)...");
		...
		КонецЕсли;
	Исключение
		ДобавитьВЛогВыгрузки("Ошибка соединения: " + ОписаниеОшибки());
	КонецПопытки;
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьВЛогВыгрузки(Сообщение)
	ЛогВыгрузки = ЛогВыгрузки + Формат(ТекущаяДатаСеанса(), "ДФ=HH:mm:ss") + "  " + Сообщение + Символы.ПС;
КонецПроцедуры
