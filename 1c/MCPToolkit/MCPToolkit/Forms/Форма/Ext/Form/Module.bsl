...
&НаКлиенте
Перем ПоследняяРегистрацияВШлюзеУспешна; // Булево, результат последнего вызова /api/register

&НаКлиенте
Перем ПоследнийHeartbeatEPF; // Число (мс unix-time) — последний успешный /api/epf-heartbeat
...
Процедура ПриОткрытии(Отказ)
	...
	ПоследняяРегистрацияВШлюзеУспешна = Ложь;
	ПоследнийHeartbeatEPF = 0;
	...
	// Принудительно прокси-режим (встроенный сервер убран из UI)
	РежимПодключения = 0;

	// Анонимизация выключена по умолчанию
	ИспользоватьТочнуюАнонимизацию = Ложь;

	// Инициализация видимости элементов по текущему режиму
	ПриИзмененииРежимаПодключения(Неопределено);

	// Автозаполнение BSL-полей из текущей базы
	ЗаполнитьПоляBSLПоУмолчанию();

	// Загрузка пути BSL исходников с шлюза
	ЗагрузитьПутьBSL();

КонецПроцедуры

&НаКлиенте
Процедура ФлагАвтоЗаписатьПриИзменении(Элемент)

КонецПроцедуры

&НаКлиенте
Процедура ФлагАвтоПривилегированныйРежимПриИзменении(Элемент)

КонецПроцедуры

&НаКлиенте
Процедура ФлагАвтоВсеОпасныеОперацииПриИзменении(Элемент)

	Если НЕ АвтоРазрешитьВсеОпасныеОперации Тогда
		Возврат;
	КонецЕсли;

	Оповещение = Новый ОписаниеОповещения(
		"ПослеПодтвержденияАвтоРазрешенияВсехОпасныхОпераций",
		ЭтаФорма
	);
	ПоказатьВопрос(
		Оповещение,
		ПолучитьТекстПодтвержденияАвтоРазрешенияВсехОпасныхОпераций(),
...
&НаКлиенте
Процедура ПослеПодтвержденияАвтоРазрешенияВсехОпасныхОпераций(Ответ, ДополнительныеПараметры) Экспорт

	Если Ответ <> КодВозвратаДиалога.Да Тогда
		АвтоРазрешитьВсеОпасныеОперации = Ложь;
	КонецЕсли;

КонецПроцедуры

&НаКлиенте
Функция ПолучитьТекстПодтвержденияАвтоРазрешенияВсехОпасныхОпераций()

	СтрокиПредупреждения = Новый Массив;
	СтрокиПредупреждения.Добавить("Вы уверены?");
	СтрокиПредупреждения.Добавить("");
	СтрокиПредупреждения.Добавить("Все действия, включая опасные операции, будут выполняться автоматически без предупреждения.");
	СтрокиПредупреждения.Добавить("");
	СтрокиПредупреждения.Добавить("Это включает:");
	СтрокиПредупреждения.Добавить("- Удаление объектов");
	СтрокиПредупреждения.Добавить("- Запись объектов");
	СтрокиПредупреждения.Добавить("- Операции с файлами и каталогами");
	СтрокиПредупреждения.Добавить("- Внешние компоненты и COM-объекты");
	СтрокиПредупреждения.Добавить("- Монопольный и привилегированный режимы");
	СтрокиПредупреждения.Добавить("");
	СтрокиПредупреждения.Добавить("Продолжить?");

	Возврат СоединитьМассив(СтрокиПредупреждения, Символы.ПС);
...
Процедура Подключиться(Команда)
	...
	СброситьСостояниеПодключенияПередПереподключением();
	СгенерироватьИдентификаторКанала(Неопределено);

	// Шаг 1: Регистрация базы в шлюзе (получение toolkit URL)
	УстановитьСтатус("Регистрация в шлюзе...", Ложь);
	ДобавитьВЛог("Регистрация базы в шлюзе...");
	ПоследняяРегистрацияВШлюзеУспешна = Ложь;
	ЗарегистрироватьсяВШлюзе(Неопределено);
	Если НЕ ПоследняяРегистрацияВШлюзеУспешна Тогда
		УстановитьСтатус("Ошибка регистрации в шлюзе", Ложь);
		Возврат;
	КонецЕсли;

	// Шаг 2: Подключение к серверу toolkit
	// Если шлюз не задал адрес — используем дефолтный (localhost:6003)
	Если ПустаяСтрока(АдресСервера) Тогда
		АдресСервера = "http://localhost:6003";
		ДобавитьВЛог("Шлюз не задал адрес сервера, используется: " + АдресСервера);
	КонецЕсли;
	...
	Попытка
		...
		ПроверкаСоединение = Новый HTTPСоединение(СтруктураURL.Хост, СтруктураURL.Порт, , , , 5);
		ПроверкаЗапрос = Новый HTTPЗапрос("/health");
		ПроверкаОтвет = ПроверкаСоединение.Получить(ПроверкаЗапрос);
		Если ПроверкаОтвет.КодСостояния <> 200 Тогда
			УстановитьСтатус("Сервер недоступен", Ложь);
			ДобавитьВЛог("Сервер " + АдресСервера + " не отвечает (HTTP " + ПроверкаОтвет.КодСостояния + ")");
			Возврат;
		КонецЕсли;
	Исключение
		УстановитьСтатус("Сервер недоступен", Ложь);
		ДобавитьВЛог("Сервер " + АдресСервера + " недоступен: " + ОписаниеОшибки());
		Возврат;
	КонецПопытки;
	...
	УстановитьСтатус("Подключение к серверу...", Ложь);
	ДобавитьВЛог("Подключение к серверу: " + АдресСервера);

	// Проверка: если gateway расположен не на localhost — BSL-функции недоступны с этой машины
	ПараметрыGatewayДляПроверки = ПолучитьПараметрыГейтвея(ГейтвейURL);
	АдресGatewayДляПроверки = ПараметрыGatewayДляПроверки.URL;
	СтруктураURLGW = РазобратьURL(АдресGatewayДляПроверки);
	ХостGateway = НРег(СтруктураURLGW.Хост);
	GatewayУдаленный = НЕ ПустаяСтрока(ХостGateway)
		И СтрНайти(ХостGateway, "localhost") = 0
		И СтрНайти(ХостGateway, "127.0.0.1") = 0;

	Если GatewayУдаленный Тогда
		ДобавитьВЛог("---");
		ДобавитьВЛог("Gateway на удалённой машине (" + СтруктураURLGW.Хост + ").");
		ДобавитьВЛог("BSL-навигация и выгрузка BSL недоступны — запустите MCP локально.");
		ДобавитьВЛог("---");
	КонецЕсли;

	ПодключитьОбработчикОжидания("ЦиклОпроса", 1, Ложь);
	ОтправитьHeartbeatEPF(Истина);

КонецПроцедуры

&НаКлиенте
Процедура СброситьСостояниеПодключенияПередПереподключением()
	...
	Исключение
		// Обработчик уже отключён или ещё не был запущен.
	КонецПопытки;
...
&НаКлиенте
Процедура УведомитьШлюзОбОтключенииEPF()
	Попытка
		ПараметрыGateway = ПолучитьПараметрыГейтвея(ГейтвейURL);
		АдресGateway = ПараметрыGateway.URL;
		Если НЕ ПустаяСтрока(АдресGateway) И НЕ ПустаяСтрока(ИмяБазы) Тогда
			СтруктураURL = РазобратьURL(АдресGateway);
			Соединение = Новый HTTPСоединение(СтруктураURL.Хост, СтруктураURL.Порт, , , , 3);
			Запрос = Новый HTTPЗапрос("/api/unregister");
			Запрос.Заголовки.Вставить("Content-Type", "application/json");
			ДобавитьАвторизациюГейтвея(Запрос, ПараметрыGateway.APIToken);
			Запрос.УстановитьТелоИзСтроки("{""name"": """ + СокрЛП(ИмяБазы) + """}", "UTF-8");
			Ответ = Соединение.ОтправитьДляОбработки(Запрос);
			Если Ответ.КодСостояния <> 200 Тогда
				ДобавитьВЛог("! Шлюз вернул HTTP " + Строка(Ответ.КодСостояния) + " при отключении EPF.");
			КонецЕсли;
		КонецЕсли;
	Исключение
		ДобавитьВЛог("! Ошибка уведомления шлюза об отключении: " + ОписаниеОшибки());
	КонецПопытки;
КонецПроцедуры
...
&НаКлиенте
Процедура ОткрытьДашборд(Команда)
	ПараметрыGateway = ПолучитьПараметрыГейтвея(ГейтвейURL);
	ЗапуститьПриложение(ПараметрыGateway.URL + "/dashboard");
КонецПроцедуры

&НаКлиенте
Процедура ОткрытьGitHub(Команда)
	ЗапуститьПриложение("https://github.com/AlekseiSeleznev/onec-mcp-universal");
КонецПроцедуры
...
Процедура ОбработатьОшибкуПодключения(ОписаниеОшибки)
	...
	Если ТекущееКоличествоПопытокПереподключения >= МаксимальноеКоличествоПопытокПереподключения Тогда
		// После серии ошибок не отключаем форму полностью:
		// пытаемся восстановить регистрацию и продолжить polling на текущем channel.
		УстановитьСтатус("Восстановление подключения...", Ложь);
		ДобавитьВЛог("Ошибка подключения: " + ОписаниеОшибки + ". Превышено количество попыток. Выполняется восстановление подключения...");
		ВосстановитьПодключениеПослеОшибки();
	Иначе
...
&НаКлиенте
Процедура ВосстановитьПодключениеПослеОшибки()
	...
	ДобавитьВЛог("Выполняется повторная регистрация базы в шлюзе для восстановления polling...");
	ЗарегистрироватьсяВШлюзе(Неопределено);

	Если ПоследняяРегистрацияВШлюзеУспешна Тогда
		ДобавитьВЛог("Регистрация восстановлена. Возобновляется опрос сервера.");
		ОтправитьHeartbeatEPF(Истина);
	Иначе
		ДобавитьВЛог("Повторная регистрация в шлюзе не удалась. Ожидается следующая попытка polling.");
	КонецЕсли;
...
Функция ВсеОперацииАвтоРазрешены(МассивОпасныхСлов)
	...
	Если АвтоРазрешитьВсеОпасныеОперации Тогда
		Возврат Истина;
	КонецЕсли;
...
&НаКлиенте
Функция НормализоватьBearerТокен(Токен)

	Нормализованный = СокрЛП(Токен);
	Если ПустаяСтрока(Нормализованный) Тогда
		Возврат "";
	КонецЕсли;

	Если НРег(Лев(Нормализованный, 7)) = "bearer " Тогда
		Нормализованный = СокрЛП(Сред(Нормализованный, 8));
	КонецЕсли;
...
&НаКлиенте
Функция ИзвлечьAPITokenИзПараметров(СтрокаПараметров)

	Параметры = СокрЛП(СтрокаПараметров);
	Если ПустаяСтрока(Параметры) Тогда
		Возврат "";
	КонецЕсли;

	Для Каждого Часть Из СтрРазделить(Параметры, "&", Ложь) Цикл
		Фрагмент = СокрЛП(Часть);
		Если ПустаяСтрока(Фрагмент) Тогда
			Продолжить;
		КонецЕсли;

		ПозицияРавно = Найти(Фрагмент, "=");
		Если ПозицияРавно = 0 Тогда
			Продолжить;
		КонецЕсли;

		Ключ = НРег(СокрЛП(Лев(Фрагмент, ПозицияРавно - 1)));
		Значение = СокрЛП(Сред(Фрагмент, ПозицияРавно + 1));

		Если Ключ = "token"
			Или Ключ = "api_token"
			Или Ключ = "gateway_api_token"
			Или Ключ = "bearer"
			Или Ключ = "auth_token" Тогда
			Возврат НормализоватьBearerТокен(Значение);
		КонецЕсли;
	КонецЦикла;
...
&НаКлиенте
Функция ПолучитьПараметрыГейтвея(URLГейтвея)

	Адрес = СокрЛП(URLГейтвея);
	...
	ПозицияРешетки = Найти(Адрес, "#");
	Если ПозицияРешетки > 0 Тогда
		Фрагмент = Сред(Адрес, ПозицияРешетки + 1);
		Адрес = Лев(Адрес, ПозицияРешетки - 1);
		Токен = ИзвлечьAPITokenИзПараметров(Фрагмент);
	КонецЕсли;

	ПозицияВопроса = Найти(Адрес, "?");
	Если ПозицияВопроса > 0 Тогда
		СтрокаПараметров = Сред(Адрес, ПозицияВопроса + 1);
		Адрес = Лев(Адрес, ПозицияВопроса - 1);
		Если ПустаяСтрока(Токен) Тогда
			Токен = ИзвлечьAPITokenИзПараметров(СтрокаПараметров);
		КонецЕсли;
	КонецЕсли;

	Возврат Новый Структура("URL, APIToken", Адрес, Токен);

КонецФункции

&НаКлиенте
Процедура ДобавитьАвторизациюГейтвея(Запрос, APIToken)

	Токен = НормализоватьBearerТокен(APIToken);
	Если ПустаяСтрока(Токен) Тогда
		Возврат;
	КонецЕсли;
...
&НаКлиенте
Функция ИзвлечьАдресСервераИзОтветаРегистрации(ТелоОтвета)

	НовыйАдрес = "";

	// Приоритет: штатный JSON-парсинг ответа шлюза.
	Попытка
		ЧтениеJSON = Новый ЧтениеJSON;
		ЧтениеJSON.УстановитьСтроку(ТелоОтвета);
		ДанныеОтвета = ПрочитатьJSON(ЧтениеJSON);
		Если ТипЗнч(ДанныеОтвета) = Тип("Соответствие")
			И ДанныеОтвета["toolkit_poll_url"] <> Неопределено Тогда
			НовыйАдрес = Строка(ДанныеОтвета["toolkit_poll_url"]);
		КонецЕсли;
	Исключение
	КонецПопытки;

	// Fallback для совместимости со старыми/нестандартными ответами.
	Если ПустаяСтрока(НовыйАдрес) Тогда
		НачалоURL = СтрНайти(ТелоОтвета, """toolkit_poll_url"":""");
		Если НачалоURL > 0 Тогда
			НачалоURL = НачалоURL + СтрДлина("""toolkit_poll_url"":""");
			КонецURL = СтрНайти(ТелоОтвета, """", НаправлениеПоиска.СНачала, НачалоURL);
			Если КонецURL > НачалоURL Тогда
				НовыйАдрес = Сред(ТелоОтвета, НачалоURL, КонецURL - НачалоURL);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Возврат СтрЗаменить(НовыйАдрес, "/1c/poll", "");
...
Функция ВыполнитьКод(Код)
	...
	Исключение
		ТекстОшибки = ОписаниеОшибки();
		Если ЭтоОшибкаДоступаКФлагамАвтоРазрешения(Код, ТекстОшибки) Тогда
			ТекстОшибки = "Флаги авторазрешения формы доступны только в client context. "
				+ "Используйте execute_code с execution_context=""client"". "
				+ "Исходная ошибка: " + ТекстОшибки;
		КонецЕсли;
...
&НаСервереБезКонтекста
Функция ЭтоОшибкаДоступаКФлагамАвтоРазрешения(Код, ТекстОшибки)

	Если ПустаяСтрока(ТекстОшибки) Тогда
		Возврат Ложь;
	КонецЕсли;

	Если Найти(ТекстОшибки, "Переменная не определена") = 0 Тогда
		Возврат Ложь;
	КонецЕсли;

	ИменаФлагов = Новый Массив;
	ИменаФлагов.Добавить("АвтоРазрешитьЗаписать");
	ИменаФлагов.Добавить("АвтоРазрешитьПривилегированныйРежим");
	ИменаФлагов.Добавить("АвтоРазрешитьВсеОпасныеОперации");

	Для Каждого ИмяФлага Из ИменаФлагов Цикл
		Если Найти(ТекстОшибки, ИмяФлага) > 0 ИЛИ Найти(Код, ИмяФлага) > 0 Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗагрузитьНастройки()
	...
	АвтоРазрешитьЗаписать = Ложь;
	АвтоРазрешитьПривилегированныйРежим = Ложь;
	АвтоРазрешитьВсеОпасныеОперации = Ложь;
...
&НаКлиенте
Процедура ОтправитьHeartbeatEPF(Принудительно = Ложь)
	Если НЕ ФлагПодключения Или ПустаяСтрока(ИмяБазы) Тогда
		Возврат;
	КонецЕсли;

	ТекущееВремяМс = ТекущаяУниверсальнаяДатаВМиллисекундах();
	Если НЕ Принудительно И ПоследнийHeartbeatEPF > 0 Тогда
		Если (ТекущееВремяМс - ПоследнийHeartbeatEPF) < 5000 Тогда
			Возврат;
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		ПараметрыGateway = ПолучитьПараметрыГейтвея(ГейтвейURL);
		АдресGateway = ПараметрыGateway.URL;
		Если НЕ ПустаяСтрока(АдресGateway) Тогда
			СтруктураURL = РазобратьURL(АдресGateway);
			Соединение = Новый HTTPСоединение(СтруктураURL.Хост, СтруктураURL.Порт, , , , 3);
			Запрос = Новый HTTPЗапрос("/api/epf-heartbeat");
			Запрос.Заголовки.Вставить("Content-Type", "application/json");
			ДобавитьАвторизациюГейтвея(Запрос, ПараметрыGateway.APIToken);
			ТелоHeartbeat = "{""name"": """ + СокрЛП(ИмяБазы) + """";
			Если НЕ ПустаяСтрока(СокрЛП(ИдентификаторКанала)) Тогда
				ТелоHeartbeat = ТелоHeartbeat + ", ""channel"": """ + СокрЛП(ИдентификаторКанала) + """";
			КонецЕсли;
			ТелоHeartbeat = ТелоHeartbeat + "}";
			Запрос.УстановитьТелоИзСтроки(ТелоHeartbeat, "UTF-8");
			Ответ = Соединение.ОтправитьДляОбработки(Запрос);
			Если Ответ.КодСостояния = 200 Тогда
				ПоследнийHeartbeatEPF = ТекущееВремяМс;
			ИначеЕсли Ответ.КодСостояния = 404 Тогда
				ПоследнийHeartbeatEPF = 0;
				ПоследняяРегистрацияВШлюзеУспешна = Ложь;
				ДобавитьВЛог("Gateway потерял регистрацию базы. Выполняется повторная регистрация...");
				ЗарегистрироватьсяВШлюзе(Неопределено);
				Если ПоследняяРегистрацияВШлюзеУспешна Тогда
					ПоследнийHeartbeatEPF = ТекущееВремяМс;
				КонецЕсли;
		...
		КонецЕсли;
	Исключение
		// heartbeat best-effort: не шумим в лог на каждом цикле опроса
	КонецПопытки;
КонецПроцедуры
...
Процедура ПриИзмененииРежимаПодключения(Элемент)
	...
	// Видимость групп
	Элементы.ГруппаПроксиНастройки.Видимость = Ложь; // Скрыто — поля на главной форме
...
Функция ОбработатьToolsCall(ДанныеЗапроса, ТелоJSON, Ид, ИдПотокаSSE = "")
	...
	Если ИмяИнструмента = "execute_code" Тогда
		КонтекстВыполнения = "server";
		Если Аргументы.Свойство("execution_context") Тогда
			КонтекстВыполнения = Аргументы.execution_context;
		КонецЕсли;
...
Функция ХотяБыОдинФлажокВключён()
	Возврат АвтоРазрешитьЗаписать ИЛИ АвтоРазрешитьПривилегированныйРежим ИЛИ АвтоРазрешитьВсеОпасныеОперации;
КонецФункции
...
&НаСервереБезКонтекста
Функция ПолучитьПараметрыТекущейБазыНаСервере()
	Результат = Новый Структура("Сервер, ИмяБазы");
	СтрокаПодключения = СтрокаСоединенияИнформационнойБазы();
	// Парсим строку подключения формата "Srvr=...;Ref=...;" или "File=...;"
	Части = СтрРазделить(СтрокаПодключения, ";", Ложь);
	Для Каждого Часть Из Части Цикл
		КлючЗначение = СтрРазделить(Часть, "=", Ложь);
		Если КлючЗначение.Количество() < 2 Тогда
			Продолжить;
		КонецЕсли;
		Ключ    = НРег(СокрЛП(КлючЗначение[0]));
		Значение = СокрЛП(СтрЗаменить(КлючЗначение[1], """", ""));
		Если Ключ = "srvr" Тогда
			Результат.Сервер = Значение;
		ИначеЕсли Ключ = "ref" Тогда
			Результат.ИмяБазы = Значение;
		КонецЕсли;
	КонецЦикла;
	Возврат Результат;
КонецФункции
...
&НаКлиенте
Процедура ОпределитьПараметрыБазы(Команда)
	ПараметрыБазы = ПолучитьПараметрыТекущейБазыНаСервере();
	СерверБазы = ПараметрыБазы.Сервер;
	ИмяБазы    = ПараметрыБазы.ИмяБазы;
	Если ПустаяСтрока(СерверБазы) Тогда
		ДобавитьВЛог("Сервер не определён — возможно, файловая база.");
	Иначе
		ДобавитьВЛог("Сервер: " + СерверБазы + ", база: " + ИмяБазы);
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ВыгрузитьBSL(Команда)
	Если ПустаяСтрока(СокрЛП(СерверБазы)) ИЛИ ПустаяСтрока(СокрЛП(ИмяБазы)) Тогда
		ДобавитьВЛог("Ошибка: параметры подключения к базе не определены автоматически.");
		Возврат;
	КонецЕсли;
	...
	СерверТримм   = СокрЛП(СерверБазы);
	БазаТримм     = СокрЛП(ИмяБазы);
	ПользТримм    = СокрЛП(ПользовательBSL);
	ПарольТримм   = СокрЛП(ПарольBSL);

	СтрокаПодключения = "Srvr=" + СерверТримм + ";Ref=" + БазаТримм + ";";
	Если НЕ ПустаяСтрока(ПользТримм) Тогда
		СтрокаПодключения = СтрокаПодключения + "Usr=" + ПользТримм + ";";
	КонецЕсли;
	Если НЕ ПустаяСтрока(ПарольТримм) Тогда
		СтрокаПодключения = СтрокаПодключения + "Pwd=" + ПарольТримм + ";";
	КонецЕсли;

	// Предварительно запрашиваем у шлюза реальный путь, куда пойдёт выгрузка,
	// и показываем пользователю подтверждение. Если preview недоступен — всё
	// равно спрашиваем подтверждение, но без конкретного пути.
	ПутьВыгрузки = "";
	Попытка
		СтруктураURLPreview = РазобратьURL(АдресGateway);
		Если НЕ ПустаяСтрока(СтруктураURLPreview.Хост) Тогда
			Если СтруктураURLPreview.Протокол = "https" Тогда
				СоединениеPreview = Новый HTTPСоединение(СтруктураURLPreview.Хост, СтруктураURLPreview.Порт, , , , 10, Новый ЗащищенноеСоединениеOpenSSL());
			Иначе
				СоединениеPreview = Новый HTTPСоединение(СтруктураURLPreview.Хост, СтруктураURLPreview.Порт, , , , 10);
			КонецЕсли;
			ЗапросPreview = Новый HTTPЗапрос("/api/export-preview");
			ЗапросPreview.Заголовки.Вставить("Content-Type", "application/json");
			ДобавитьАвторизациюГейтвея(ЗапросPreview, ПараметрыGateway.APIToken);
			ЗапросPreview.УстановитьТелоИзСтроки("{""connection"": """ + СтрокаПодключения + """}", "UTF-8");
			ОтветPreview = СоединениеPreview.ОтправитьДляОбработки(ЗапросPreview);
			Если ОтветPreview.КодСостояния = 200 Тогда
				ЧтениеPreview = Новый ЧтениеJSON;
				ЧтениеPreview.УстановитьСтроку(ОтветPreview.ПолучитьТелоКакСтроку("UTF-8"));
				ДанныеPreview = ПрочитатьJSON(ЧтениеPreview);
				Если ТипЗнч(ДанныеPreview) = Тип("Соответствие") И ДанныеPreview["output_path_host"] <> Неопределено Тогда
					ПутьВыгрузки = Строка(ДанныеPreview["output_path_host"]);
				ИначеЕсли ТипЗнч(ДанныеPreview) = Тип("Структура") И ДанныеPreview.Свойство("output_path_host") Тогда
					ПутьВыгрузки = Строка(ДанныеPreview.output_path_host);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	...
	Если НЕ ПустаяСтрока(ПутьВыгрузки) Тогда
		ТекстВопроса = "Выгрузить BSL в:" + Символы.ПС + ПутьВыгрузки + Символы.ПС + Символы.ПС + "Продолжить?";
	Иначе
		ТекстВопроса = "Выгрузить BSL в папку, настроенную на дашборде?" + Символы.ПС + "(путь уточняется шлюзом)" + Символы.ПС + Символы.ПС + "Продолжить?";
	КонецЕсли;

	ДанныеПродолжения = Новый Структура;
	ДанныеПродолжения.Вставить("АдресGateway", АдресGateway);
	ДанныеПродолжения.Вставить("ПараметрыGateway", ПараметрыGateway);
	ДанныеПродолжения.Вставить("СтрокаПодключения", СтрокаПодключения);
	ДанныеПродолжения.Вставить("СерверТримм", СерверТримм);
	ДанныеПродолжения.Вставить("БазаТримм", БазаТримм);
	ДанныеПродолжения.Вставить("ПутьВыгрузки", ПутьВыгрузки);

	Оповещение = Новый ОписаниеОповещения("ВыгрузитьBSL_ПослеПодтверждения", ЭтотОбъект, ДанныеПродолжения);
	ПоказатьВопрос(Оповещение, ТекстВопроса, РежимДиалогаВопрос.ДаНет);
КонецПроцедуры

&НаКлиенте
Процедура ВыгрузитьBSL_ПослеПодтверждения(Ответ, ДанныеПродолжения) Экспорт
	Если Ответ <> КодВозвратаДиалога.Да Тогда
		ДобавитьВЛог("Выгрузка BSL отменена пользователем.");
		Возврат;
	КонецЕсли;

	АдресGateway      = ДанныеПродолжения.АдресGateway;
	ПараметрыGateway  = ДанныеПродолжения.ПараметрыGateway;
	СтрокаПодключения = ДанныеПродолжения.СтрокаПодключения;
	СерверТримм       = ДанныеПродолжения.СерверТримм;
	БазаТримм         = ДанныеПродолжения.БазаТримм;

	ДобавитьВЛог("Запуск выгрузки BSL... Подключение: Srvr=" + СерверТримм + ";Ref=" + БазаТримм + ";");

	ТелоJSON = "{""connection"": """ + СтрокаПодключения + """, ""output_dir"": ""/projects""}";

	Попытка
		СтруктураURL = РазобратьURL(АдресGateway);
		Если ПустаяСтрока(СтруктураURL.Хост) Тогда
			ДобавитьВЛог("Ошибка: неверный адрес MCP Gateway.");
			Возврат;
		КонецЕсли;
		...
		КонецЕсли;
			...
			Запрос = Новый HTTPЗапрос("/api/export-bsl");
			Запрос.Заголовки.Вставить("Content-Type", "application/json");
			ДобавитьАвторизациюГейтвея(Запрос, ПараметрыGateway.APIToken);
			Запрос.УстановитьТелоИзСтроки(ТелоJSON, "UTF-8");
		...
		Ответ = Соединение.ОтправитьДляОбработки(Запрос);
			...
			Если Ответ.КодСостояния = 200 Тогда
				...
				Попытка
					...
					ЧтениеJSON200 = Новый ЧтениеJSON;
					ЧтениеJSON200.УстановитьСтроку(ТелоОК);
					ДанныеОК = ПрочитатьJSON(ЧтениеJSON200);
					Если ТипЗнч(ДанныеОК) = Тип("Соответствие") Тогда
						Если ДанныеОК["output_path_host"] <> Неопределено Тогда
							ПутьВыгрузки = Строка(ДанныеОК["output_path_host"]);
						КонецЕсли;
					ИначеЕсли ТипЗнч(ДанныеОК) = Тип("Структура") Тогда
						Если ДанныеОК.Свойство("output_path_host") Тогда
							ПутьВыгрузки = Строка(ДанныеОК.output_path_host);
						КонецЕсли;
					КонецЕсли;
				Исключение
					ДобавитьВЛог("Диагностика: не удалось прочитать путь выгрузки из ответа: " + ОписаниеОшибки());
				КонецПопытки;
				Если НЕ ПустаяСтрока(ПутьВыгрузки) Тогда
					ДобавитьВЛог("Путь выгрузки: " + ПутьВыгрузки);
				КонецЕсли;
				ДобавитьВЛог("Выгрузка запущена в фоне. Нажмите ""Статус выгрузки"" для проверки.");
			ИначеЕсли Ответ.КодСостояния = 409 Тогда
				ДобавитьВЛог("Выгрузка уже выполняется.");
			ИначеЕсли Ответ.КодСостояния = 401 Или Ответ.КодСостояния = 403 Тогда
				ДобавитьВЛог("Gateway требует API токен. Укажите его в поле «Адрес шлюза» как #token=ваш_токен.");
			Иначе
			...
			Попытка
				ЧтениеJSON = Новый ЧтениеJSON;
				ЧтениеJSON.УстановитьСтроку(ТелоОтвета);
				// ПрочитатьJSON возвращает Соответствие, не Структуру
				ДанныеОтвета = ПрочитатьJSON(ЧтениеJSON);
				Если ТипЗнч(ДанныеОтвета) = Тип("Соответствие") Тогда
					Если ДанныеОтвета["error"] <> Неопределено Тогда
						ТекстОшибки = ДанныеОтвета["error"];
					ИначеЕсли ДанныеОтвета["result"] <> Неопределено Тогда
						ТекстОшибки = ДанныеОтвета["result"];
					КонецЕсли;
				КонецЕсли;
			Исключение
			КонецПопытки;
			ДобавитьВЛог("" + Символы.ПС + "! ОШИБКА: " + ТекстОшибки + Символы.ПС + "");
		КонецЕсли;
	Исключение
		ДобавитьВЛог("" + Символы.ПС + "! ОШИБКА соединения с Gateway: " + ОписаниеОшибки() + Символы.ПС + "");
	КонецПопытки;
КонецПроцедуры

&НаКлиенте
Процедура СтатусВыгрузкиBSL(Команда)
	ПараметрыGateway = ПолучитьПараметрыГейтвея(ГейтвейURL);
	АдресGateway = ПараметрыGateway.URL;

	СтрокаПодключения = "";
	Если НЕ ПустаяСтрока(СокрЛП(СерверБазы)) И НЕ ПустаяСтрока(СокрЛП(ИмяБазы)) Тогда
		СтрокаПодключения = "Srvr=" + СокрЛП(СерверБазы) + ";Ref=" + СокрЛП(ИмяБазы) + ";";
		Если НЕ ПустаяСтрока(СокрЛП(ПользовательBSL)) Тогда
			СтрокаПодключения = СтрокаПодключения + "Usr=" + СокрЛП(ПользовательBSL) + ";";
		КонецЕсли;
		Если НЕ ПустаяСтрока(СокрЛП(ПарольBSL)) Тогда
			СтрокаПодключения = СтрокаПодключения + "Pwd=" + СокрЛП(ПарольBSL) + ";";
		КонецЕсли;
	КонецЕсли;
	...
	Попытка
		...
		Если СтруктураURL.Протокол = "https" Тогда
			Соединение = Новый HTTPСоединение(СтруктураURL.Хост, СтруктураURL.Порт, , , , 15, Новый ЗащищенноеСоединениеOpenSSL());
		Иначе
			Соединение = Новый HTTPСоединение(СтруктураURL.Хост, СтруктураURL.Порт, , , , 15);
		КонецЕсли;

			Запрос = Новый HTTPЗапрос("/api/export-status");
			Запрос.Заголовки.Вставить("Content-Type", "application/json");
			ДобавитьАвторизациюГейтвея(Запрос, ПараметрыGateway.APIToken);
			Запрос.УстановитьТелоИзСтроки("{""connection"": """ + СтрокаПодключения + """}", "UTF-8");
			Ответ = Соединение.ОтправитьДляОбработки(Запрос);
		...
		Если Ответ.КодСостояния = 200 Тогда
			Попытка
				...
				СтруктураСтатуса = ПрочитатьJSON(ЧтениеJSON);
				// ПрочитатьJSON возвращает Соответствие — используем ["ключ"]
				СтатусВыгрузки = СтруктураСтатуса["status"];
				РезультатВыгрузки = СтруктураСтатуса["result"];
				СтатусИндекса   = ?(СтруктураСтатуса["index_status"] <> Неопределено, СтруктураСтатуса["index_status"], "idle");
				РезультатИндекса = ?(СтруктураСтатуса["index_result"] <> Неопределено, СтруктураСтатуса["index_result"], "");

				Если СтатусВыгрузки = "idle" Тогда
					ДобавитьВЛог("Выгрузка BSL: не запускалась.");
				ИначеЕсли СтатусВыгрузки = "running" Тогда
					Если СтатусИндекса = "running" Тогда
						ДобавитьВЛог("Идёт индексация BSL для поиска по коду...");
					Иначе
						ДобавитьВЛог("Выгрузка BSL: идёт... Это может занять продолжительное время — зависит от размера конфигурации.");
					КонецЕсли;
				ИначеЕсли СтатусВыгрузки = "done" Тогда
					// Парсим "Export completed: N BSL files in ..."
					КоличествоФайлов = "";
					ПозицияBSL = СтрНайти(РезультатВыгрузки, "BSL files");
					Если ПозицияBSL > 0 Тогда
						ФрагментДо = Лев(РезультатВыгрузки, ПозицияBSL - 2);
						НачалоЧисла = СтрДлина(ФрагментДо);
						Пока НачалоЧисла > 0 И Сред(ФрагментДо, НачалоЧисла, 1) <> " " Цикл
							НачалоЧисла = НачалоЧисла - 1;
						КонецЦикла;
						КоличествоФайлов = " (" + Сред(ФрагментДо, НачалоЧисла + 1) + " BSL файлов)";
					КонецЕсли;
					ДобавитьВЛог("Выгрузка BSL: завершена успешно" + КоличествоФайлов + ".");

					// Статус индексации
					Если СтатусИндекса = "running" Тогда
						ДобавитьВЛог("Идёт индексация BSL для поиска по коду...");
					ИначеЕсли СтатусИндекса = "done" Тогда
						Если НЕ ПустаяСтрока(РезультатИндекса) Тогда
							ДобавитьВЛог("Индексация завершена: " + РезультатИндекса + ".");
						Иначе
							ДобавитьВЛог("Индексация завершена.");
						КонецЕсли;
					ИначеЕсли СтатусИндекса = "error" Тогда
						ДобавитьВЛог("! Ошибка индексации BSL: " + РезультатИндекса);
					ИначеЕсли СтатусИндекса = "idle" Тогда
						ДобавитьВЛог("Индексация не выполнялась (перезапустите выгрузку).");
					КонецЕсли;
				ИначеЕсли СтатусВыгрузки = "cancelled" Тогда
					ДобавитьВЛог("Выгрузка BSL: отменена пользователем.");
				ИначеЕсли СтатусВыгрузки = "error" Тогда
					Если НЕ ПустаяСтрока(РезультатВыгрузки) Тогда
						ДобавитьВЛог("" + Символы.ПС + "! ОШИБКА выгрузки BSL: " + РезультатВыгрузки + Символы.ПС + "");
					Иначе
						ДобавитьВЛог("" + Символы.ПС + "! ОШИБКА выгрузки BSL." + Символы.ПС + "");
					КонецЕсли;
				КонецЕсли;
			Исключение
				ДобавитьВЛог("Статус выгрузки: " + Ответ.ПолучитьТелоКакСтроку());
			КонецПопытки;
		Иначе
			ДобавитьВЛог("" + Символы.ПС + "! ОШИБКА HTTP " + Ответ.КодСостояния + Символы.ПС + "");
		КонецЕсли;
	Исключение
		ДобавитьВЛог("" + Символы.ПС + "! ОШИБКА соединения с Gateway: " + ОписаниеОшибки() + Символы.ПС + "");
	КонецПопытки;
...
&НаКлиенте
Процедура ОтменитьВыгрузкуBSL(Команда)
	Если ПустаяСтрока(СокрЛП(СерверБазы)) ИЛИ ПустаяСтрока(СокрЛП(ИмяБазы)) Тогда
		ДобавитьВЛог("Ошибка: параметры подключения к базе не определены.");
		Возврат;
	КонецЕсли;
	...
	Попытка
		...
		КонецЕсли;
			...
			Запрос = Новый HTTPЗапрос("/api/export-cancel");
			Запрос.Заголовки.Вставить("Content-Type", "application/json");
			ДобавитьАвторизациюГейтвея(Запрос, ПараметрыGateway.APIToken);
			Запрос.УстановитьТелоИзСтроки("{""connection"": """ + СтрокаПодключения + """}", "UTF-8");
			Ответ = Соединение.ОтправитьДляОбработки(Запрос);
			...
			Если Ответ.КодСостояния = 200 Тогда
				ДобавитьВЛог("Запрос на отмену выгрузки BSL отправлен.");
			ИначеЕсли Ответ.КодСостояния = 404 Тогда
				ДобавитьВЛог("Нет активной выгрузки BSL для этой базы.");
			ИначеЕсли Ответ.КодСостояния = 401 Или Ответ.КодСостояния = 403 Тогда
				ДобавитьВЛог("Gateway требует API токен. Укажите его в поле «Адрес шлюза» как #token=ваш_токен.");
			Иначе
				ДобавитьВЛог("" + Символы.ПС + "! ОШИБКА HTTP " + Ответ.КодСостояния + Символы.ПС + "");
...
&НаКлиенте
Процедура ЗаполнитьПоляBSLПоУмолчанию()
	// Заполняем поля только если они пустые (не перезатираем сохранённые значения)
	Если ПустаяСтрока(СокрЛП(СерверБазы)) И ПустаяСтрока(СокрЛП(ИмяБазы)) Тогда
		ПараметрыБазы = ПолучитьПараметрыТекущейБазыНаСервере();
		Если НЕ ПустаяСтрока(ПараметрыБазы.Сервер) Тогда
			СерверБазы = ПараметрыБазы.Сервер;
			ИмяБазы    = ПараметрыБазы.ИмяБазы;
		КонецЕсли;
	КонецЕсли;
	Если ПустаяСтрока(СокрЛП(ГейтвейURL)) Тогда
		ГейтвейURL = "http://localhost:8080";
	КонецЕсли;
	// Автозаполнение пользователя текущим сеансом
	Если ПустаяСтрока(СокрЛП(ПользовательBSL)) Тогда
		ПользовательBSL = ИмяПользователя();
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ЗарегистрироватьсяВШлюзе(Команда)
	ПараметрыGateway = ПолучитьПараметрыГейтвея(ГейтвейURL);
	АдресGateway = ПараметрыGateway.URL;
	ПоследняяРегистрацияВШлюзеУспешна = Ложь;

	ИмяБазыДляРегистрации = СокрЛП(ИмяБазы);
	Если ПустаяСтрока(ИмяБазыДляРегистрации) Тогда
		ДобавитьВЛог("Ошибка: имя базы не заполнено. Нажмите «Определить автоматически».");
		Возврат;
	КонецЕсли;

	СтрокаПодключенияРег = "Srvr=" + СокрЛП(СерверБазы) + ";Ref=" + ИмяБазыДляРегистрации + ";";

	ТелоJSON = "{""name"": """ + ИмяБазыДляРегистрации + """, ""connection"": """ + СтрокаПодключенияРег + """, ""channel"": """ + СокрЛП(ИдентификаторКанала) + """}";

	Попытка
		СтруктураURL = РазобратьURL(АдресGateway);
			Если СтруктураURL.Протокол = "https" Тогда
				Соединение = Новый HTTPСоединение(СтруктураURL.Хост, СтруктураURL.Порт, , , , 240, Новый ЗащищенноеСоединениеOpenSSL());
			Иначе
				Соединение = Новый HTTPСоединение(СтруктураURL.Хост, СтруктураURL.Порт, , , , 240);
			КонецЕсли;

			Запрос = Новый HTTPЗапрос("/api/register");
			Запрос.Заголовки.Вставить("Content-Type", "application/json");
			ДобавитьАвторизациюГейтвея(Запрос, ПараметрыGateway.APIToken);
			Запрос.УстановитьТелоИзСтроки(ТелоJSON, "UTF-8");
			...
			Ответ = Соединение.ОтправитьДляОбработки(Запрос);
				Если Ответ.КодСостояния = 200 Тогда
					ДобавитьВЛог("База """ + ИмяБазыДляРегистрации + """ зарегистрирована в шлюзе");
					ПоследняяРегистрацияВШлюзеУспешна = Истина;

				// Переключаемся на toolkit URL, выданный шлюзом (динамический порт).
				ТелоОтвета = Ответ.ПолучитьТелоКакСтроку();
				НовыйАдресСервера = ИзвлечьАдресСервераИзОтветаРегистрации(ТелоОтвета);
				Если НЕ ПустаяСтрока(НовыйАдресСервера) Тогда
					АдресСервера = НовыйАдресСервера;
					ДобавитьВЛог("Адрес сервера: " + АдресСервера);
				Иначе
					ДобавитьВЛог("Шлюз не вернул toolkit_poll_url, используется текущий адрес сервера.");
				КонецЕсли;
				ИначеЕсли Ответ.КодСостояния = 401 Или Ответ.КодСостояния = 403 Тогда
					ДобавитьВЛог("Gateway требует API токен. Укажите его в поле «Адрес шлюза» как #token=ваш_токен.");
				Иначе
					ДобавитьВЛог("Ошибка регистрации HTTP " + Ответ.КодСостояния + ": " + Ответ.ПолучитьТелоКакСтроку());
				КонецЕсли;
	Исключение
		ДобавитьВЛог("Ошибка подключения к шлюзу: " + ОписаниеОшибки());
	КонецПопытки;
КонецПроцедуры

// ─────────────────────────────────────────────────────────────────────────────
// НАСТРОЙКА ПУТИ BSL ИСХОДНИКОВ
// ─────────────────────────────────────────────────────────────────────────────

&НаКлиенте
Процедура ЗагрузитьПутьBSL()
	ПараметрыGateway = ПолучитьПараметрыГейтвея(ГейтвейURL);
	АдресGateway = ПараметрыGateway.URL;
	...
	Попытка
		...
		Если СтруктураURL.Протокол = "https" Тогда
			Соединение = Новый HTTPСоединение(СтруктураURL.Хост, СтруктураURL.Порт, , , , 5, Новый ЗащищенноеСоединениеOpenSSL());
		Иначе
			Соединение = Новый HTTPСоединение(СтруктураURL.Хост, СтруктураURL.Порт, , , , 5);
		КонецЕсли;

			Запрос = Новый HTTPЗапрос("/api/action/get-bsl-workspace");
			ДобавитьАвторизациюГейтвея(Запрос, ПараметрыGateway.APIToken);
			Ответ = Соединение.ОтправитьДляОбработки(Запрос);
		...
		Если Ответ.КодСостояния = 200 Тогда
			...
			Если ТипЗнч(Данные) = Тип("Структура") И Данные.Свойство("value") Тогда
				ПутьBSL = Данные.value;
			КонецЕсли;
		КонецЕсли;
	Исключение
		// Не критично — Gateway может быть недоступен
	КонецПопытки;
КонецПроцедуры

&НаКлиенте
Процедура ВыбратьПапкуBSL(Команда)
	Диалог = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.ВыборКаталога);
	Диалог.Заголовок = "Выберите папку для BSL исходников";
	Если Не ПустаяСтрока(ПутьBSL) Тогда
		Диалог.Каталог = ПутьBSL;
	КонецЕсли;
	Если Диалог.Выбрать() Тогда
		ПутьBSL = Диалог.Каталог;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура СохранитьПутьBSL(Команда)
	ПараметрыGateway = ПолучитьПараметрыГейтвея(ГейтвейURL);
	АдресGateway = ПараметрыGateway.URL;

	НовыйПуть = СокрЛП(ПутьBSL);
	...
	Попытка
		...
		ТелоJSON = "{""value"": """ + СтрЗаменить(НовыйПуть, """", "\""") + """}";

			Запрос = Новый HTTPЗапрос("/api/action/save-bsl-workspace");
			Запрос.Заголовки.Вставить("Content-Type", "application/json");
			ДобавитьАвторизациюГейтвея(Запрос, ПараметрыGateway.APIToken);
			Запрос.УстановитьТелоИзСтроки(ТелоJSON, "UTF-8");
		...
		Ответ = Соединение.ОтправитьДляОбработки(Запрос);
			...
			Если Ответ.КодСостояния = 200 Тогда
				ДобавитьВЛог("Путь BSL сохранён: " + НовыйПуть + ". Шлюз перезапускается...");
			ИначеЕсли Ответ.КодСостояния = 401 Или Ответ.КодСостояния = 403 Тогда
				ДобавитьВЛог("Gateway требует API токен. Укажите его в поле «Адрес шлюза» как #token=ваш_токен.");
			Иначе
				ДобавитьВЛог("Ошибка сохранения пути BSL HTTP " + Ответ.КодСостояния + ": " + Ответ.ПолучитьТелоКакСтроку());
			КонецЕсли;
	Исключение
		ДобавитьВЛог("Ошибка соединения с Gateway: " + ОписаниеОшибки());
	КонецПопытки;
КонецПроцедуры
