...
```powershell
python .claude/skills/db-dump-cf/scripts/db-dump-cf.py <параметры>
```
...
python .claude/skills/db-dump-cf/scripts/db-dump-cf.py -InfoBasePath "C:\Bases\MyDB" -UserName "Admin" -OutputFile "C:\backup\config.cf"

# Серверная база
python .claude/skills/db-dump-cf/scripts/db-dump-cf.py -InfoBaseServer "srv01" -InfoBaseRef "MyApp_Dev" -UserName "Admin" -Password "secret" -OutputFile "config.cf"

# Выгрузка расширения
python .claude/skills/db-dump-cf/scripts/db-dump-cf.py -InfoBasePath "C:\Bases\MyDB" -UserName "Admin" -OutputFile "ext.cfe" -Extension "МоёРасширение"
```
