...
```powershell
python .claude/skills/mxl-info/scripts/mxl-info.py -TemplatePath "<путь>"
```

Или по имени обработки/макета:
```powershell
python .claude/skills/mxl-info/scripts/mxl-info.py -ProcessorName "<Имя>" -TemplateName "<Макет>" [-SrcDir "<каталог>"]
```
