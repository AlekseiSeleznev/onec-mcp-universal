...
python .claude/skills/epf-dump/scripts/epf-dump.py -InfoBasePath "C:\Bases\MyDB" -InputFile "build/МойОтчёт.erf" -OutputDir "src"

# Серверная база
python .claude/skills/epf-dump/scripts/epf-dump.py -InfoBaseServer "srv01" -InfoBaseRef "MyDB" -UserName "Admin" -Password "secret" -InputFile "build/МойОтчёт.erf" -OutputDir "src"
```
