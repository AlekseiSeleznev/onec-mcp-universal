...
# Cross-platform: Windows and Linux (uses xvfb-run on Linux if available)
...
def _v8cmd(v8path):
    """Wrap v8path with xvfb-run on Linux if available."""
    if platform.system() != "Windows":
        xvfb = shutil.which("xvfb-run")
        if xvfb:
            return [xvfb, "--auto-servernum", "--server-args=-screen 0 1024x768x24", v8path]
    return [v8path]
...
"""
    ...
    result = subprocess.run(
        _v8cmd(args.V8Path) + ['CREATEINFOBASE', f'File={temp_base}', '/DisableStartupDialogs'],
    ...
    if has_ref_types:
        ...
        result = subprocess.run(
            _v8cmd(args.V8Path) + ['DESIGNER', f'/F{temp_base}', '/LoadConfigFromFiles', cfg_dir, '/DisableStartupDialogs'],
            capture_output=True, text=True,
        )
        if result.returncode != 0:
        ...
        result = subprocess.run(
            _v8cmd(args.V8Path) + ['DESIGNER', f'/F{temp_base}', '/UpdateDBCfg', '/Out', update_log, '/DisableStartupDialogs'],
            capture_output=True, text=True,
        )
        if result.returncode != 0:
