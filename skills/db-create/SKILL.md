...
```powershell
python .claude/skills/db-create/scripts/db-create.py <параметры>
```
...
python .claude/skills/db-create/scripts/db-create.py -InfoBasePath "C:\Bases\NewDB"

# Создать серверную базу
python .claude/skills/db-create/scripts/db-create.py -InfoBaseServer "srv01" -InfoBaseRef "MyApp_Test"

# Создать из шаблона CF
python .claude/skills/db-create/scripts/db-create.py -InfoBasePath "C:\Bases\NewDB" -UseTemplate "C:\Templates\config.cf"

# Создать и добавить в список баз
python .claude/skills/db-create/scripts/db-create.py -InfoBasePath "C:\Bases\NewDB" -AddToList -ListName "Новая база"
```
