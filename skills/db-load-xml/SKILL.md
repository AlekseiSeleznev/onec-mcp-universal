...
```powershell
python .claude/skills/db-load-xml/scripts/db-load-xml.py <параметры>
```
...
```powershell
# Полная загрузка
python .claude/skills/db-load-xml/scripts/db-load-xml.py -V8Path "C:\Program Files\1cv8\8.3.25.1257\bin" -InfoBasePath "C:\Bases\MyDB" -UserName "Admin" -ConfigDir "C:\WS\cfsrc" -Mode Full

# Частичная загрузка конкретных файлов
python .claude/skills/db-load-xml/scripts/db-load-xml.py -InfoBasePath "C:\Bases\MyDB" -UserName "Admin" -ConfigDir "C:\WS\cfsrc" -Mode Partial -Files "Catalogs/Номенклатура.xml,Catalogs/Номенклатура/Ext/ObjectModule.bsl"

# Загрузка расширения
python .claude/skills/db-load-xml/scripts/db-load-xml.py -InfoBasePath "C:\Bases\MyDB" -UserName "Admin" -ConfigDir "C:\WS\ext_src" -Mode Full -Extension "МоёРасширение"

# Загрузка + обновление БД в одном запуске
python .claude/skills/db-load-xml/scripts/db-load-xml.py -InfoBasePath "C:\Bases\MyDB" -UserName "Admin" -ConfigDir "C:\WS\cfsrc" -Mode Full -UpdateDB
```
