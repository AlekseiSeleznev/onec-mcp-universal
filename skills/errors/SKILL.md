---
name: error-handling
description: Обработка ошибок, транзакции и блокировки в 1С. КРИТИЧЕСКИ ВАЖНО для целостности данных.
---
...
- Пишешь код с `НачатьТранзакцию()`
- Работаешь с записью данных в БД
- Используешь блокировки (`БлокировкаДанных`, `ЗаблокироватьДанныеДляРедактирования`)
- Обрабатываешь исключения (`Попытка/Исключение`)
- Делаешь массовые операции с данными

## Критический принцип

В 1С транзакции управляются ВРУЧНУЮ. Незакрытая транзакция = потеря данных или зависшая блокировка.
Каждая ошибка в транзакциях может ПОЛНОСТЬЮ остановить работу системы.

---

## БЫСТРАЯ СПРАВКА: Приоритеты правил

🔴 КРИТИЧНО (нарушение = потеря данных):
- Правило 2: Канонический паттерн транзакций
- Правило 3: Проверка ТранзакцияАктивна()
- Правило 1: Всегда логировать исключения

🟡 ВАЖНО (нарушение = проблемы производительности/блокировки):
- Правило 4: Минимизация времени транзакции
- Правило 5: БлокировкаДанных перед чтением
- Правило 11: Единый порядок блокировки

🟢 РЕКОМЕНДУЕТСЯ (нарушение = плохой UX/отладка):
- Правило 7: Структурированное логирование
- Правило 8: Понятные сообщения пользователю
- Правило 10: Поэлементная обработка массовых операций
...
- [ ] Каждый блок `Попытка/Исключение` содержит `ЗаписьЖурналаРегистрации()`
- [ ] Используется `ПодробноеПредставлениеОшибки()` для журнала
- [ ] Используется `КраткоеПредставлениеОшибки()` для пользователя
