...
```powershell
python skills/db-run/scripts/db-run.py <параметры>
```
...
```powershell
# Простой запуск
python skills/db-run/scripts/db-run.py -InfoBasePath "C:\Bases\MyDB" -UserName "Admin"

# Запуск с обработкой
python skills/db-run/scripts/db-run.py -InfoBasePath "C:\Bases\MyDB" -UserName "Admin" -Execute "C:\epf\МояОбработка.epf"

# Открыть по навигационной ссылке
python skills/db-run/scripts/db-run.py -InfoBasePath "C:\Bases\MyDB" -UserName "Admin" -URL "e1cib/data/Справочник.Номенклатура"

# Серверная база с параметром запуска
python skills/db-run/scripts/db-run.py -InfoBaseServer "srv01" -InfoBaseRef "MyDB" -UserName "Admin" -Password "secret" -CParam "ЗапуститьОбновление"
```
