...
`onec-mcp-universal` — памятка Codex для MCP-шлюза 1С:Предприятия.

> **Правила работы AI-ассистента** описаны в [`AGENTS.md`](AGENTS.md) (секция «Протокол работы AI-ассистента»). MCP-сервер также возвращает агрегированный `instructions`-блок при `initialize` — современные MCP-клиенты подмешивают его в системный промпт автоматически. Ниже — только Codex-специфичные шаги развёртывания.
...
- **Если пользователь просит получить данные отчёта**, в первую очередь используй report-tools `onec-mcp-universal`, а не прямые запросы к регистрам: `find_reports(database, query)` или `list_reports(database)` → `describe_report(database, title|report|variant)` → `run_report(database, title|report, period, filters, params, context)` → при большом результате `get_report_result(database, run_id)`. Если пользовательская формулировка не находится или отчёт плохо описан, сначала используй `enrich_report_docs(database, report|title|query)` — он подтягивает описание из ИТС/1С:Напарника в per-db каталог. Если `run_report` вернул `needs_input`, дозаполни только явно запрошенные поля и повтори `run_report`. Для report-tools `database` обязателен; не полагайся на активную БД сессии. Пользователь говорит «Расчетный листок»/«расчетка», а не `Отчет.АнализНачисленийИУдержаний`.
